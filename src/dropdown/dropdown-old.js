...
.directive('dropdown', function () {
    return {
        ...
        link: function (scope, element, attrs, DropDownController) {
            scope.isSearch=true;
            var dropdown_class =['ui','selection', 'dropdown'];

            if(attrs.isSearch){alert(0);
                dropdown_class.push('search');
            }
            scope.dropdown_class = dropdown_class.join(' ');
            ...
            scope.$watch('model', function (val) {
                ...
                scope.model = val;
                DropDownController.update_value(val || scope.original_title);
            });
...
.directive('dropdownGroup', function () {
    return {
        ...
        template: '<div class="item" ng-transclude data-value={{item_value}} >{{ item_title }}</div>',
        link: function (scope, element, attrs, DropDownController) {
            ...
            element.bind('click', function () {
                ...
                DropDownController.update_value(scope.item_value);
